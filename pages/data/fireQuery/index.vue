  ...
  <view class="fire-upload-page">
    <view class="form-container">
      ...
      <view class="form-section">
        <view class="form-row">
          <text class="section-title">救灾单位 <text class="required">*</text></text>
      ...
      <view class="form-section">
        <view class="form-row">
          <text class="section-title">救援车辆 <text class="required">*</text></text>
          <picker :value="fireCarIndex" :range="fireCarOptions" range-key="label" @change="onFireCarChange" class="form-picker">
            <view class="picker-display">
              <text class="picker-text" :class="{'placeholder':!formData.fireCar}">{{ getFireCarText }}</text>
              <image :src="serverUrl + '/static/icons/common/down.png'" class="picker-arrow" />
            </view>
          </picker>
        </view>
      ...
      <view class="form-section" v-if="formData.taskType && currentTaskConfig">
        <view class="form-row">      
          <!-- 配置：uiType = select（单选） -->
          <view v-if="currentTaskConfig.uiType === 'select'" class="form-picker-section">
          ...
          <view v-else-if="currentTaskConfig.uiType === 'select-collection'" class="form-picker-section">
      ...
      <view class="form-section">
        <view class="form-row">
          ...
          <picker :value="taskStatusIndex" :range="taskStatusOptions" range-key="label" @change="onTaskStatusChange" class="form-picker">
            <view class="picker-display">
              <text class="picker-text" :class="{'placeholder':!formData.taskStatus}">{{ getTaskStatusText }}</text>
              <image :src="serverUrl + '/static/icons/common/down.png'" class="picker-arrow" />
            </view>
          </picker>
        </view>
  ...
  methods: {
    loadStaticData() {
      try {
        ...
        const taskStatuses = uni.getStorageSync('static_taskStatuses') || []
        ...
        if (fireCars.length > 0) {
          this.formData.fireCar = fireCars[0].value
        }
        if (taskTypes.length > 0) {
          ...
          if (this.currentTaskConfig && this.currentTaskConfig.actionKey) {
            ...
            this.$set(this.formData.taskExtra, this.currentTaskConfig.actionKey, this.currentTaskConfig.options ? this.currentTaskConfig.options[0] : '')
          }
        }
        if (taskStatuses.length > 0) {
          this.formData.taskStatus = taskStatuses[0].value
        }
      } catch (e) {
    ...
    onFireCarChange(e) { 
      this.fireCarIndex = Number(e.detail.value)
      this.formData.fireCar = this.fireCarOptions[this.fireCarIndex]?.value || '' 
    },
    onDirectionChange(e) { 
    ...
    onTaskStatusChange(e) { 
      this.taskStatusIndex = Number(e.detail.value)
      this.formData.taskStatus = this.taskStatusOptions[this.taskStatusIndex]?.value || '' 
    },
    ...
    async submitForm() {
      ...
      if (!fd.fireUnit) {
        uni.showToast({ title: '请选择消防单位', icon: 'none' })
        return
      }
      if (!fd.fireCar) {
        uni.showToast({ title: '请选择消防车辆', icon: 'none' })
        return
      }
      if (!fd.addressId) {
      ...
      if (fd.locationType === 1) {
        if (!fd.rescueFloor || !/^[1-9]\d{0,1}$|^100$/.test(String(fd.rescueFloor))) {
          uni.showToast({ title: '请输入正确的搜救楼层(1-100)', icon: 'none' })
          return
        }
      } else {
        if (!fd.direction) {
...
.input-container { flex: 1; margin-left: 24rpx; }
